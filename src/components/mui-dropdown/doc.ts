  ...
  Dropdown: {
    ...
    usage: {
      list: [
        ...
        "Menu width is optional. Without width, Dropdown sizes the Menu to its widest item, including icons and inset padding, then clamps it to the available viewport; author Menu width when a composition needs a deliberate measure.",
    ...
    stories: {
      items: [
        { key: "action-sizes", title: "Action Sizes", list: ["Set size on Dropdown; it keeps the trigger and direct Menu actions aligned.", "Choose the size that matches the surrounding controls and available space.", "A portaled Menu without an explicit width sizes to its widest item and remains viewport-safe; set width on Menu when the composition needs a deliberate measure."] },
        { key: "intrinsic-width", title: "Intrinsic Menu Width", description: "Exercises content-based, viewport-safe Menu measurement without an authored width.", list: ["Menu width is optional; the Dropdown measures its widest item once when opening and keeps ResizeObserver updates position-only.", "Icons, action gaps, padding, and Menu inset contribute to the intrinsic width.", "Use the status and dynamic-item control to check both content measurement and the window-level ResizeObserver regression."] },
        ...
        { key: "menu-width", title: "Menu Width", description: "Set an explicit width for the composed Menu surface.", list: ["Without an explicit width, Dropdown measures the widest Menu item and caps the portaled surface to the available viewport.", "Pass any valid CSS width to Menu using the width attribute when the composition needs a deliberate width rather than content-based sizing.", "Use min(100%, value) to cap the preferred width within its available viewport space."] },

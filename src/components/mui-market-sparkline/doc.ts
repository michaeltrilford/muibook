...
export const muiDocs: MuiDocs = {
  MarketSparkline: {
    ...
    guides: [""],
    usage: { list: ["Use for compact market summaries where direction and shape matter more than exact axis inspection.", "Time and value scales are hidden by default. Set scale to both, time, or price when the compact chart needs a bottom time scale, right-side value scale, or both.", "Compose Sparkline inside Card when it needs a framed dashboard surface; the component remains unframed for use within existing panels.", "Provide time/value objects through the data property. When composing Muibook UI in Redactd, populate the structured Data field through props.data; Redactd passes it to the component.", "Compose headings, values, badges, and actions through Muibook components in the header and footer slots.", "In a chart cluster, use attribution='none' to remove repeated TradingView logos only when the containing experience provides one visible attribution and the required NOTICE information.", "Use Financial Chart when users need ranges, OHLC values, or detailed market analysis."] },
    ...
    stories: { items: [
      ...
      { key: "scales", title: "Scales", list: ["Leave scale unset for the default axis-free Sparkline.", "Use both, time, or price only when the compact trend needs the bottom time scale, right-side value scale, or both.", "Use a fuller financial chart when axis inspection becomes the primary task."] },
    ...
    rules: [],
    behaviour: { list: ["The plot resizes with its host using the chart engine's native auto-size behavior.", "Changing scale updates the visible axes without rebuilding the supplied series.", "Changing type, trend, baseline, or theme redraws the series without changing the supplied data.", "Interactive is opt-in so compact dashboard charts do not capture scrolling or pointer gestures by default."] },

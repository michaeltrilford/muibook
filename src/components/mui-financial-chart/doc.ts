...
export const muiDocs: MuiDocs = {
  FinancialChart: {
    ...
    guides: [""],
    usage: { list: ["Use candlesticks when open, high, low, and close values matter; use area for a simpler price trend.", "Compose Financial Chart inside Card when the chart needs a framed surface; Financial Chart remains unframed so it can also fit existing panels and full-width layouts.", "Provide OHLCV objects through the data property rather than an HTML attribute. When composing Muibook UI in Redactd, populate the structured Data field through props.data; Redactd passes it to the component.", "Keep the TradingView logo enabled by default. In a chart cluster, use attribution='none' to remove repeated logos only when the containing experience provides one visible TradingView attribution and the required NOTICE information."] },

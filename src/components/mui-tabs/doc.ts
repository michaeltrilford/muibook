  ...
  Tabs: {
    ...
    stories: {
      items: [
        ...
        { key: "surface-usage", title: "Surface Usage", description: "Adapts the Tab Bar hierarchy when composed inside a framed surface.", list: ["Card, Card Body, Drawer, Dialog, and Carousel automatically apply usage='surface' through neutral layout wrappers while preserving the nearest nested surface owner, including dynamically mounted and remounted Tab Bars.", "Surface usage applies the base surface to the Tab Bar background and border, with the elevated surface behind the active tab."] },
        { key: "keyed-dialog", title: "Keyed Dialog Content", description: "Replaces a Dialog Tab Bar as a framework key changes.", list: ["Change the Tab Bar key while the Dialog is open to remount its instance.", "The replacement Tab Bar retains the Dialog surface treatment without consumer-authored usage state."] },
        ...
        { key: "value", title: "Declarative Value", description: "Selects a tab from application state without an imperative element reference.", list: ["Set value to the id of a direct Tab Item.", "When value is present it is the canonical selection; use child active only for an uncontrolled initial selection."] },
        { key: "active-attribute", title: "Child Active Attribute", description: "Responds when a framework updates active directly on a Tab Item.", list: ["Use this for existing framework integrations that render active on child Tab Items.", "Prefer value on the Tab Bar for new controlled integrations."] },
        { key: "disabled", title: "Disabled Tab", description: "Keeps unavailable views visible without allowing selection.", list: ["Use disabled for temporarily unavailable views such as an Outcomes share link.", "Disabled tabs cannot be selected and are skipped by arrow-key navigation."] },
